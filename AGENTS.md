...
- Git, projects under `05-PROJECTS/`: real codebases keep the branch flow — `/commit-push-pr` ships, `/merge-pr` lands once checks are green. Never force-push, anywhere. ONE exception (ruled 2026-08-03): harness files (`AGENTS.md`, `CLAUDE.md` — the synced dirs are project-gitignored) are auto-committed and pushed straight to the project's default branch by `sync-project-harness.sh`, but only when the repo is already on main/master; on any other branch the sync leaves them uncommitted and reports — never checkout, the parked branch may be another session's work.
