...
- Eight agents in `.claude/agents/` — SOURCE OF TRUTH for all agent capability: builders (development, frontend), judges (code-review, web-qa, security — read+test only, never write source, never judge their own runtime's build), operators (shopify, browser, marketing). Obsidian vault work needs no agent (retired 2026-08-11) — the claude-obsidian + vault skills carry it at main level. No specialist fit → main level. `model:` in each definition sets its tier and wins over the CLI default for a `--agent` pane — opus on code-review/security, sonnet elsewhere; never spawn a worker wave on a premium tier.
...
- Project state: GitHub Issues + label state machine (ruled 2026-08-11) — the work-order store; Projects v2 board is the human kanban view, never authoritative. Protocol + label sync: `01-HARNESS/PROJECT-STATE.md` (`sync-wo-labels.sh` seeds `wo:*`/`agent:*` labels on every remoted repo). Linear rejected: `06-REFERENCE/project-state-linear-vs-github.md`. Engine v0 (label-driven `run.sh`) designed but NOT built — refining first: `06-REFERENCE/engine-synthesis.md`.
