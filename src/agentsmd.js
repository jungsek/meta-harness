...
export function emitAgentsMd(model, targetNames, warnings = []) {
  if (!targetNames.some((t) => AGENTS_MD_TARGETS.includes(t))) return []
  const all = model.rules.filter((r) => targetNames.some((t) => AGENTS_MD_TARGETS.includes(t) && wants(r, t)))
  // A `paths:`-scoped rule is conditional in Claude — it loads only when a
  // matching file is opened. AGENTS.md has no conditional form, so copying one
  // here would silently promote it to always-on. Skip it and say so.
  const rules = all.filter((r) => !r.fm.paths && !r.fm.globs)
  for (const r of all.filter((r) => r.fm.paths || r.fm.globs))
    warnings.push(
      `AGENTS.md: rule "${r.name}" is path-scoped — skipped, since AGENTS.md loads unconditionally and would apply it everywhere`
    )
