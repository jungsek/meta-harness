...
const RULES_DOC = '.codex/harness-rules.md'
...
export default {
  ...
  emit(model, ctx) {
    ...
    // Rules reach Codex through a project doc we own outright, registered via
    // `project_doc_fallback_filenames`. Verified against codex 0.145: those
    // files load *in addition to* AGENTS.md, not instead of it — so the user's
    // AGENTS.md is never touched, and we get whole-file ownership.
    // (.codex/rules/*.rules is Starlark exec policy, a different mechanism.)
    // A `paths:`-scoped rule is conditional in Claude — it loads only when a
    // matching file is opened. Codex has no conditional-loading equivalent, so
    // copying one here would silently promote it to always-on context. Skip it
    // and say so rather than quietly changing what the user asked for.
    const all = model.rules.filter((r) => wants(r, 'codex'))
    const scoped = all.filter((r) => r.fm.paths || r.fm.globs)
    const rules = all.filter((r) => !r.fm.paths && !r.fm.globs)
    for (const r of scoped)
      ctx.warnings.push(
        `codex: rule "${r.name}" is path-scoped — skipped, since Codex loads project docs unconditionally and would apply it everywhere`
      )
    if (rules.length) {
      // `root: true` leads: identity should be read before the rules it frames.
      const ordered = [...rules.filter((r) => r.fm.root), ...rules.filter((r) => !r.fm.root)]
      out.push({
        category: 'rules',
        path: RULES_DOC,
        content:
          ['<!-- Generated by meta-harness from your rules. Edit them at the source, not here. -->', '']
            .concat(ordered.map((r) => r.body.trim()))
            .join('\n') + '\n',
      })
      out.push({
        category: 'rules',
        ...
        format: 'toml',
        data: { project_doc_fallback_filenames: [RULES_DOC] },
      })
    }
