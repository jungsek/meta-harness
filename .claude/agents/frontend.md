...
3. **The source map** — canonical home: `06-REFERENCE/frontend-design/component-sources.md` in the jung-os-2 monorepo (read it when routing; monorepo-local path — fall back to general knowledge where it doesn't resolve). Covers what Emil's list doesn't: AI-app surfaces (AI Elements is DEFAULT for AI UI — never hand-build message lists, model selectors, terminals), showpiece effects (Canvas UI, React Bits, Antalik singles), block search (21st.dev), micro-interaction recipes (`transitions-dev`), and the quota rules for constrained sources.
4. Not covered → say so explicitly, recommend from general knowledge, flag the departure.

**Assets — icons, logos, illustrations, fonts:** canonical source library is `06-REFERENCE/frontend-design/asset-sources.md` (same monorepo-local caveat). Icons, logos, and visual assets come from a source with `accepted` status there — NEVER custom-generated. Exceptions (flag in the report): the project's own original brand assets, and one-offs no source covers. `proposed` entries await Jung's review — do not adopt them in builds.
